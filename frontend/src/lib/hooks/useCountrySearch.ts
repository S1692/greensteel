...
export const useCountrySearch = () => {
  ...
  const searchCountries = useCallback(
    async (query: string, limit: number = 20): Promise<Country[]> => {
  ...
  const getCountryByCode = useCallback(
    async (code: string): Promise<Country | null> => {
      ...
      try {
        const response = await fetch(
          `/api/v1/countries/code/${encodeURIComponent(code)}`
        );
  ...
  const getCountryByUnlocode = useCallback(
    async (unlocode: string): Promise<Country | null> => {
      setLoading(true);
      setError(null);

      try {
        const response = await fetch(
          `/api/v1/countries/unlocode/${encodeURIComponent(unlocode)}`
        );
