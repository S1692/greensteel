...
async def ai_process_stream(data: dict):
    ...
    try:
        ...
        logger.info(f"받은 데이터 구조: {type(input_data)}")
        logger.info(f"받은 컬럼: {columns}")
        if input_data:
            logger.info(f"첫 번째 행 데이터: {input_data[0] if len(input_data) > 0 else 'None'}")
        ...
        def generate_ai_recommendation(row: dict) -> str:
            """규칙 기반 AI 추천 답변 생성"""
            공정 = row.get('공정', '') or ''
            투입물명 = row.get('투입물명', '') or ''
            수량 = row.get('수량', 0) or 0
            단위 = row.get('단위', '') or ''
            
            # 문자열로 변환하여 안전하게 처리
            공정_str = str(공정).lower()
            투입물명_str = str(투입물명).lower()
            수량_str = str(수량)
            단위_str = str(단위)
            ...
            if '코크스' in 공정_str:
                if '석탄' in 투입물명_str:
                    return f"코크스 제조 공정에 적합한 석탄 사용. 수량: {수량_str} {단위_str}"
                else:
                    return f"코크스 공정에 적합한 원료 선택 필요. 현재: {투입물명_str}"
            
            elif '소결' in 공정_str:
                if '철광석' in 투입물명_str or '광석' in 투입물명_str:
                    return f"소결 공정에 적합한 철광석 사용. 수량: {수량_str} {단위_str}"
                else:
                    return f"소결 공정에 적합한 원료 선택 필요. 현재: {투입물명_str}"
            
            elif '제철' in 공정_str:
                if '코크스' in 투입물명_str or '석탄' in 투입물명_str:
                    return f"제철 공정에 적합한 연료 사용. 수량: {수량_str} {단위_str}"
                else:
                    return f"제철 공정에 적합한 연료 선택 필요. 현재: {투입물명_str}"
            
            elif '제강' in 공정_str:
                if '산소' in 투입물명_str or '가스' in 투입물명_str:
                    return f"제강 공정에 적합한 산화제 사용. 수량: {수량_str} {단위_str}"
                else:
                    return f"제철 공정에 적합한 산화제 선택 필요. 현재: {투입물명_str}"
            
            elif '압연' in 공정_str:
                if '전기' in 투입물명_str or '에너지' in 투입물명_str:
                    return f"압연 공정에 적합한 에너지 사용. 수량: {수량_str} {단위_str}"
                else:
                    return f"압연 공정에 적합한 에너지 선택 필요. 현재: {투입물명_str}"
            
            else:
                return f"일반 공정. 원료: {투입물명_str}, 수량: {수량_str} {단위_str}"
