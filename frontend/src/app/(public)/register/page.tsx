...
export default function RegisterPage() {
  ...
  const handleCompanySubmit = async (e: React.FormEvent) => {
    ...
    // 필수 필드 검증
    if (!companyData.name_ko || !companyData.biz_no || !companyData.manager_name || !companyData.manager_phone) {
    ...
    try {
      const response = await axiosClient.post("/auth/register/company", companyData);
      setSuccess(`기업 등록이 완료되었습니다! 기업 ID: ${response.data.id}`);
      
      // 기업 ID를 User 회원가입에 설정
      setUserData(prev => ({ ...prev, company_id: response.data.id.toString() }));
      setActiveTab("user");
    } catch (err: unknown) {
  ...
  const handleUserSubmit = async (e: React.FormEvent) => {
    ...
    // 유효성 검사
    if (!userData.username || !userData.password || !userData.full_name || !userData.company_id) {
        ...
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          {activeTab === "company" && (
            <form onSubmit={handleCompanySubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    사업자(상점) 국문 이름 <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
              ...
              <h3 className="text-lg font-medium text-gray-900 mt-6 mb-4">주소 정보</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">광역 도시명</label>
                  <input
                    type="text"
              ...
              <h3 className="text-lg font-medium text-gray-900 mt-6 mb-4">업종 정보</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">업태/업종</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">업종 코드</label>
                  <input
                    type="text"
              ...
              <h3 className="text-lg font-medium text-gray-900 mt-6 mb-4">담당자 정보</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
