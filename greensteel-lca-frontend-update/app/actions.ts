...
export async function updateProject(projectId: string, projectData: Partial<ProjectMeta>) {
...
export async function saveScope(projectId: string, formData: ProjectMeta & AnalysisScope) {
  ...
  return { success: true, message: "Scope and project metadata saved successfully" }
}
...
export async function startLciaRun(projectId: string, config: { method: string; categories: string[] }) {
  ...
  if (result.success) {
    return {
      success: true,
      runId: result.data?.runId || `run-${Date.now()}`,
...
export async function requestReport(projectId: string, format: 'pdf' | 'docx' | 'html' = 'pdf') {
  console.log("[ACTION] Generating report for project:", projectId, format)
  
  const result = await apiCall(`/projects/${projectId}/report/generate`, 'POST', {
    format,
    timestamp: new Date().toISOString(),
    includeCharts: true,
    includeTables: true
...
export async function uploadProcessDiagram(projectId: string, file: File) {
  console.log("[ACTION] Uploading process diagram for project:", projectId, file.name)
