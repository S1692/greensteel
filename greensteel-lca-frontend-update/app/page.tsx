...
// import { ProjectList } from "@/components/organisms/ProjectList" // Not used in this implementation
...
export default function DashboardPage() {
  ...
  const latestProject = mockProjects.toSorted((a, b) => 
    ...
    <div className="min-h-screen">
      <div className="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">진행 중인 LCA 프로젝트를 관리하세요</p>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex justify-between items-center p-4 rounded-lg bg-card border border-border/30 shadow-sm">
              <div>
                <span className="text-muted-foreground font-medium">전체 프로젝트</span>
                <p className="text-xs text-muted-foreground">활성 프로젝트</p>
              </div>
              <span className="text-foreground font-bold text-2xl">{mockProjects.length}개</span>
            </div>
            <div className="flex justify-between items-center p-4 rounded-lg bg-card border border-border/30 shadow-sm">
              <div>
                <span className="text-muted-foreground font-medium">진행 중</span>
            ...
            <div className="bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20 rounded-xl p-6">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-foreground mb-2">{latestProject.name}</h3>
                  <p className="text-muted-foreground mb-3">{latestProject.description}</p>
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <span className="text-muted-foreground">
                      최종 업데이트: {new Date(latestProject.updatedAt).toLocaleDateString('ko-KR')}
                    </span>
                  </div>
                </div>
                <button
        ...
        {/* 프로젝트 목록 섹션 */}
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-foreground mb-4">모든 프로젝트</h2>
