  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setError(err.response?.data?.detail || err.message || '파일 업로드 중 오류가 발생했습니다');
      } else {
        ...
        {uploadResult && (
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900">업로드 성공</h3>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-gray-50 rounded-lg p-4">
                <h4 className="font-medium text-gray-700 mb-2">파일 정보</h4>
                <p className="text-sm text-gray-600">파일명: {uploadResult.data.filename}</p>
                <p className="text-sm text-gray-600">
                  크기: {uploadResult.data.shape[0]}행 × {uploadResult.data.shape[1]}열
                </p>
              </div>
              
              <div className="bg-gray-50 rounded-lg p-4">
                <h4 className="font-medium text-gray-700 mb-2">데이터 요약</h4>
                <p className="text-sm text-gray-600">총 행 수: {uploadResult.data.rows_count}</p>
                <p className="text-sm text-gray-600">총 열 수: {uploadResult.data.columns.length}</p>
              </div>
            </div>
